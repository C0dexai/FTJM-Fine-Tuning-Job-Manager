...
interface JobListItemProps {
  ...
  onAction: (jobId: string, action: 'cancel' | 'pause' | 'resume' | 'requeue') => void;
}
...
const JobListItem: React.FC<JobListItemProps> = ({ job, onSelect, onAction }) => {
  const handleAction = useCallback((e: React.MouseEvent, action: 'cancel' | 'pause' | 'resume' | 'requeue') => {
    e.stopPropagation();
  ...
  const canRequeue = ['succeeded', 'failed'].includes(job.status);
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
        ...
        <div className="md:col-span-2 flex justify-end items-center space-x-2">
          ...
          {canRequeue && (
            <Button size="sm" variant="secondary" onClick={(e) => handleAction(e, 'requeue')} title="Requeue Job" leftIcon={<RequeueIcon />} />
          )}
        </div>
      </div>
    </div>
